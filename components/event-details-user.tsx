...
export function EventDetailsUser({
  isOpen,
  onClose,
  event
}: EventDetailsUserProps) {
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="text-lg font-bold">
            ${displayPrice(Number(event.price) || 0)} USD (
            {(Number(event.price) / 1e9).toFixed(4)} SOL)
          </div>
          <div className="text-sm text-gray-600">
            <span className="font-semibold">
              {event.ticketQuantity - event.ticketsMinted}
            </span>{' '}
            tickets remaining
          </div>
          <div>
          ...
          >
            {isLoading
              ? 'Processing...'
              : `Reserve ${ticketCount} Ticket${
                  ticketCount > 1 ? 's' : ''
                } - $${(
                  Number(displayPrice(Number(event.price) || 0)) * ticketCount
                ).toFixed(2)}`}
          </Button>
        </div>
      </DialogContent>
